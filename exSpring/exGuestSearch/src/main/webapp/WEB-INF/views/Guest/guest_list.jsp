...
<c:forEach var="guest" items="${list}">
        ...
        <font face="돋움" size="2" color="#000000">
		     ...
		     <a href="guestHits?idx=${guest.idx}">${guest.subject}</a></td><!-- 제목을 클릭할 때 -->
				...
				<table>
					...
					<!-- 검색어를 이용하여 글제목, 작성자, 글내용 중에 하나를 입력 받아 처리하기 위한 부분 -->
						<tr>
							<td>
								<select name="search">
									<option value="subject" <c:if test="${vo.search=='subject'}">selected</c:if>>글제목</option>
									<option value="name" <c:if test="${vo.search=='name'}">selected</c:if>>작성자</option>
									<option value="contents" <c:if test="${vo.search=='contents'}">selected</c:if>>글내용</option>
								</select>
							</td>
							<td> <input type="text" size=20 name="key" value="${vo.key}"></td> <!--  -->
